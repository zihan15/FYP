...
class IntrinsicDataset(Dataset):
    def __init__(self, data_root, split="train", extract_type="normal", size=512):
...
for epoch in range(EPOCHS):
    ...
    for images, normals, masks in progress:
        ...
        with autocast():
            ...
            resized_masks = F.interpolate(masks, size=decoded_images.shape[-2:], mode="nearest")
            masks3 = resized_masks.expand_as(decoded_images)

            diff = (decoded_images - normals) * masks3
            loss = (diff ** 2).sum() / masks3.sum().clamp(min=1.0)
    ...
    # ===== Save after each epoch (.bin format) =====
    epoch_bin_path = os.path.join(OUTPUT_DIR, f"lora_normal_epoch_{epoch+1}.bin")
    unet.save_attn_procs(epoch_bin_path)
    print(f"Saved LoRA weights for epoch {epoch+1} to {epoch_bin_path}")
    print(f"Epoch {epoch+1} Train Loss: {train_loss:.4f}")
    
final_bin_path = os.path.join(OUTPUT_DIR, "lora_normal_final.bin")
unet.save_attn_procs(final_bin_path)
print(f"Training complete. Final LoRA weights saved to {final_bin_path}")
...
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "loss_per_step.png"))
plt.close()
print(f"Step loss plot saved to {OUTPUT_DIR}/loss_per_step.png")
...
plt.close()
print(f"Epoch loss plot saved to {OUTPUT_DIR}/loss_curve.png")
